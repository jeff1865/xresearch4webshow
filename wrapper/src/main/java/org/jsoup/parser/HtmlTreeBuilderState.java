...
enum HtmlTreeBuilderState {
    ...
    BeforeHead {
        boolean process(Token t, HtmlTreeBuilder tb) {
            if (isWhitespace(t)) {
                ...
                Element head = tb.insert(t.asStartTag(), t);
            ...
            switch (t.type) {
                ...
                case StartTag:
                    ...
                    if (name.equals("html")) {
                        ...
                        Element el = tb.insertEmpty(start, t);
                        // jsoup special: update base the frist time it is seen
                        if (name.equals("base") && el.hasAttr("href"))
                            tb.maybeSetBaseUri(el);
                    } else if (name.equals("meta")) {
                        Element meta = tb.insertEmpty(start, t);
                        // todo: charset switches
                    } else if (name.equals("title")) {
                        handleRcData(start, tb, t);
                    } else if (StringUtil.in(name, "noframes", "style")) {
                        handleRawtext(start, tb, t);
    ...
    InHeadNoscript {
        ...
        private boolean anythingElse(Token t, HtmlTreeBuilder tb) {
            tb.error(this);
            tb.insert(new Token.Character().data(t.toString()), t);
            return true;
        }
    },
    AfterHead {
            ...
            switch (t.type) {
                ...
                case StartTag:
                    ...
                    if (name.equals("a")) {
                        ...
                        tb.reconstructFormattingElements();
                        Element a = tb.insert(startTag, t);
                        tb.pushActiveFormattingElements(a);
                    } else if (StringUtil.inSorted(name, Constants.InBodyStartEmptyFormatters)) {
                        tb.reconstructFormattingElements();
                        tb.insertEmpty(startTag, t);
                        tb.framesetOk(false);
                        ...
                        tb.reconstructFormattingElements();
                        Element el = tb.insert(startTag, t);
                        ...
                        tb.reconstructFormattingElements();
                        Element el = tb.insertEmpty(startTag, t);
            ...
            if (t.isCharacter()) {
                ...
                if (name.equals("caption")) {
                    ...
                    if (tb.getFormElement() != null)
                        ...
                        tb.insertForm(startTag, false, t);
                    }
                } else {
                    return anythingElse(t, tb);
                }
                    ...
                    if (tb.getPendingTableCharacters().size() > 0) {
                        for (String character : tb.getPendingTableCharacters()) {
                            if (!isWhitespace(character)) {
                                ...
                                tb.insert(new Token.Character().data(character), t);
                        }
                        tb.newPendingTableCharacters();
                    }
                    tb.transition(tb.originalState());
    ...
    AfterBody {
        boolean process(Token t, HtmlTreeBuilder tb) {
            if (isWhitespace(t)) {
                ...
                tb.insert(t.asComment(), t); // into html node
            } else if (t.isDoctype()) {
                tb.error(this);
                return false;
            } else if (t.isStartTag() && t.asStartTag().normalName().equals("html")) {
    ...
    private static void handleRcData(Token.StartTag startTag, HtmlTreeBuilder tb, Token t) {
        tb.insert(startTag, t);
        tb.tokeniser.transition(TokeniserState.Rcdata);
        tb.markInsertionMode();
        tb.transition(Text);
    ...
    private static void handleRawtext(Token.StartTag startTag, HtmlTreeBuilder tb, Token t) {
        tb.insert(startTag, t);
