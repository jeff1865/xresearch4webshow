...
public class HtmlTreeBuilder extends TreeBuilder {
    ...
    Element insert(Token.StartTag startTag, Token token) {
        // handle empty unknown tags
        // when the spec expects an empty tag, will directly hit insertEmpty, so won't generate this fake end tag.
        if (startTag.isSelfClosing()) {
            Element el = insertEmpty(startTag, token);
            stack.add(el);
    ...
    Element insertStartTag(String startTagName, Token token) {
    ...
    Element insertEmpty(Token.StartTag startTag, Token token) {
    ...
    FormElement insertForm(Token.StartTag startTag, boolean onStack, Token token) {
        Tag tag = Tag.valueOf(startTag.name(), settings);
        FormElement el = new FormElement(tag, baseUri, startTag.attributes);
        el.setTokenIndex(token.getTokenIsndex());
        setFormElement(el);
        insertNode(el);
        if (onStack)
            stack.add(el);
    ...
    void insert(Token.Comment commentToken, Token token) {
        Comment comment = new Comment(commentToken.getData(), baseUri);
        comment.setTokenIndex(token.getTokenIsndex());
        insertNode(comment);
    }

    void insert(Token.Character characterToken, Token token) {
        Node node;
        ...
        if(node != null) node.setTokenIndex(token.getTokenIsndex());
    ...
    void reconstructFormattingElements() {
        ...
        while(true) {
            ...
            Element newEl = insertStartTag(entry.nodeName(), null); // todo: avoid fostering here?
